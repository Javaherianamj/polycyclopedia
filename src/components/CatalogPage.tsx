...
export const CatalogPage: React.FC<CatalogPageProps> = ({
  ...
  const [expandedFamilies, setExpandedFamilies] = useState<string[]>(
    initialFamily ? [initialFamily] : ['Polyolefins'],
  );
    ...
    <div className="max-w-[1080px] mx-auto px-4 py-8">
      ...
      <div className="bg-bg-surface border border-border-subtle rounded-lg p-6 sm:p-8 mb-8 text-center relative overflow-hidden shadow-xs">
        ...
        <div className="flex justify-center mb-3">
          <img
            src="/logo.svg"
            alt="Polymer Engineering Association Logo"
        ...
        <p className="text-xs sm:text-sm text-text-secondary leading-relaxed max-w-2xl mx-auto mb-6 font-medium">
          مرجع کامل خواص مهندسی، آکادمیک و صنعتی پلیمرها. برای مشاهده مشخصات کامل هر گرید، خانواده
          پلیمری را انتخاب کرده یا مشخصه مورد نظر را جستجو نمایید.
        </p>
        ...
        {Object.keys(families).length === 0 ? (
          <div className="text-center py-12 bg-bg-surface border border-border-subtle rounded-lg shadow-xs">
            <p className="text-text-secondary text-sm font-semibold">
              هیچ پلیمری با عبارت جستجو شده یافت نشد.
            </p>
          </div>
        ) : (
                      ...
                      >
                        ...
                        <div className="flex-wrap gap-2 pt-2 border-t border-border-subtle">
                          <span className="text-[11px] bg-bg-surface border border-border-subtle px-2 py-0.5 rounded text-text-secondary font-medium">
                            Tg:{' '}
                            <span className="en-mono font-mono tabular-nums font-bold text-accent-secondary">
                              {formatVal(p.thermal.tg)}
                            </span>
                          </span>
                          <span className="text-[11px] bg-bg-surface border border-border-subtle px-2 py-0.5 rounded text-text-secondary font-medium">
                            Tm:{' '}
                            <span className="en-mono font-mono tabular-nums font-bold text-status-success">
                              {formatVal(p.thermal.tm)}
                            </span>
                          </span>
                          <span className="text-[11px] bg-bg-surface border border-border-subtle px-2 py-0.5 rounded text-text-secondary font-medium">
                            Density:{' '}
                            <span className="en-mono font-mono tabular-nums font-bold text-status-warning">
                              {formatVal(p.physical.density)}
                            </span>
                          </span>
                        </div>
                      </button>
