...
- [ ] `web/package.json` — Astro, `@astrojs/react`, `react`, `react-dom`,
...
- [ ] `web/astro.config.mjs` — `output: 'static'` (R23); i18n:
...
- [ ] `web/.env.example` — `PUBLIC_API_BASE_URL` (defaults to
      `http://localhost:3001`, the API's dev port per `api/.env.example`).
- [ ] Root `.gitignore` — add `.astro/` (Astro's cache directory; `node_modules/`
      and `dist/` already match at any depth).
- [ ] `api/.env.example` — add `http://localhost:4321` (Astro's default dev
...
- [ ] Copy `design/fe-0/tokens/colour.css`, `type.css`, `motion.css` into
      `web/src/styles/tokens/` **verbatim** — per `tokens/README.md`, FE-1
      consumes these, it does not re-derive them. Import order preserved:
      colour → type → motion.
- [ ] `web/src/styles/global.css` — imports the three token files, then the
...
- [ ] Fonts: Estedad Variable + Newsreader via the same Google Fonts `@import`
...
- [ ] `web/src/i18n/config.ts` — locale list, default locale, direction map
      (`fa: 'rtl'`, `en: 'ltr'`).
- [ ] `web/src/i18n/fa.json` — shell strings only (site name, the four states'
      copy, nav placeholders). **Authored first**, per R15.
- [ ] `web/src/i18n/en.json` — same keys, translated second, verified against
      the Persian rather than the reverse.
- [ ] `web/src/i18n/t.ts` — typed lookup (`t(locale, key)`); a missing key
...
- [ ] `web/src/layouts/BaseLayout.astro` — sets `<html lang>` and `dir` from
...
- [ ] `web/src/pages/fa/index.astro`, `web/src/pages/en/index.astro` —
...
- [ ] `web/src/pages/index.astro` — redirect target for the root-redirect
...
- [ ] `web/src/lib/api/types.ts` — hand-written TS types matching the API's
      ...
      as-is.
- [ ] `web/src/lib/api/client.ts` — isomorphic `fetch` wrapper (runs at Astro
...
- [ ] No property list, section order, or display formatting is hardcoded here
      (R2/R3) — the client is a typed passthrough, nothing more.

## 6. Four states as shared components (R4)

- [ ] `web/src/components/states/Loading.astro` — respects
      `prefers-reduced-motion` (reads the motion tokens from step 2),
      `aria-live="polite"`.
- [ ] `web/src/components/states/ErrorState.astro` — a real fetch failure,
...
- [ ] `web/src/components/states/EmptyState.astro` — slot-based so FE-3/FE-4/etc
      supply their own copy; this component owns only the layout.
- [ ] `web/src/components/states/DataBoundary.tsx` — a small React helper for
...
- [ ] `web/src/lib/api/client.test.ts` — mocked `fetch`; one test per endpoint
      function covering the success path, the 404/error path, and the
      empty-list path.
- [ ] `web/src/i18n/t.test.ts` — both locales resolve every key; a missing key
      throws.
- [ ] Build-level check (step 9, not a unit test): `astro build` must produce
      `dist/fa/index.html` and `dist/en/index.html`.

## 8. Documentation

- [ ] `web/README.md` — dev setup, pointing `PUBLIC_API_BASE_URL` at the local
...
- [ ] Root `README.md` — add a `web/` row to the Repository layout table;
...
- [ ] `astro build` — confirm both locale trees exist in `dist/`.
- [ ] `npm test` — green.
- [ ] Dev server up, both `/fa/` and `/en/` loaded in the browser pane: correct
      `dir`, tokens visibly applied, theme toggle works without flash.
- [ ] No horizontal overflow at 320/375/768 (R27), both themes — same
...
- [ ] `aidlc-state.md` — new `CONSTRUCTION — FE-1 app-shell` section.
- [ ] `inception/plans/frontend-plan.md` — mark FE-1 status.
- [ ] `audit.md` — plan-approval prompt/response, then a completion entry.
