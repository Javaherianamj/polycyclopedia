...
- Polymorphic `subject_id` cannot use a plain FK. Integrity is enforced by a trigger validating existence in the target table. *(Alternative considered: separate `material_property_value`/`grade_property_value` tables. Rejected — it doubles every query and every index for the same guarantee.)*
...
**CHECK (the critical constraint)**: `locator` must be a non-empty object containing at least one of `page`, `table`, `figure`, `section`. A citation that cannot say *where* in the document is not a citation.
