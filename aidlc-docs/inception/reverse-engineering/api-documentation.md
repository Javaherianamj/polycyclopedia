...
- **Validation**: None at runtime. TypeScript's structural typing only guarantees the shape at compile time for whoever hand-authors a new record in `polymersData.ts`; a malformed literal (as happened with `thermal.cte`'s split unit, see `code-structure.md`) still type-checks as long as the field is *some* string.
