...
Note: the box width above is 67 characters, consistent within each box. There is no server tier, no database, and no authentication layer anywhere in this diagram — everything below the browser is a single static JS/CSS bundle produced by `vite build`.
...
- **Business Dictionary**:
  ...
  | **SourcedValue** | The `{ value, unit, sourceId, note? }` shape used for nearly every quantitative field, intended to carry citation provenance. In the current data, `sourceId` is **always** the literal string `'src_default'` (269 occurrences, verified by `grep -c sourceId src/data/polymersData.ts`) — i.e. the citation system is a type-level placeholder with no real source IDs and no UI that reads the field (see Code Quality Assessment). |
