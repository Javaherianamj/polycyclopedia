...
- **FR-1.2** Adding a new property (e.g. "swelling ratio" for hydrogels, "compression set" for elastomers) MUST be an `INSERT`, requiring no schema migration and no application code change. _This is the direct implementation of "leave the space for later properties added."_
